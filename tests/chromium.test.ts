...
describe("Helper", () => {
  ...
  describe("createSymlink", () => {
    it("should create a symlink when the source file exists", async () => {
      ...
      const sourceFile = join(tempDir, `source_${Date.now()}.txt`);
      const targetLink = join(tempDir, `target_${Date.now()}.txt`);
    ...
    it("should reject if the source file does not exist", async () => {
      const tempDir = tmpdir();
      const sourceFile = join(tempDir, `nonexistent_${Date.now()}.txt`);
      const targetLink = join(tempDir, `target_${Date.now()}.txt`);
  ...
  describe("downloadFile", () => {
    it("should download a file successfully", async () => {
      ...
      const tempDir = tmpdir();
      const destPath = join(tempDir, `download_test_${Date.now()}.txt`);
    ...
    it("should reject when status code is not 200", async () => {
      ...
      ).rejects.toBe("Unexpected status code: 404.");
    });
  });
